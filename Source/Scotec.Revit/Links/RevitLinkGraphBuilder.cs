...
public static class RevitLinkGraphBuilder
{
    ...
    /// The accumulated <see cref="Transform"/> representing the transformation from the host document to the current document.
    /// </param>
    /// <param name="result">
    /// The list to which discovered <see cref="RevitLinkGraphNode"/> objects are added. Each node represents a linked document 
    /// and includes its associated link instance and cumulative transformation.
    /// </param>
    /// <param name="recursionStack">
    /// A set used to track documents currently being processed in the recursion stack. This prevents cycles in the link graph.
    /// </param>
    /// <remarks>
    /// This method ensures that cycles in the link graph are detected and avoided, preventing infinite recursion.
    /// It processes each linked document, calculates its transformation relative to the host, and adds it to the result list.
    /// </remarks>
    private static void CollectLinksRecursive(Document currentDoc, Transform accumulatedTransform, List<RevitLinkGraphNode> result, HashSet<Document> recursionStack)
    {
