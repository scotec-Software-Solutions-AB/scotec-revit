...
Every availability-check scope always contains:
...
`CategorySet` and `UIApplication` are passed through directly to the attributed method and do not need to be in the DI scope.

The `Document` and `UiDocument` properties on `IRevitUiContext` may be `null` when no document is currently open.
...
> **Note:** When `UseNewScope` is `false`, `IRevitContext`, `IRevitUiContext`, and all other objects registered via `ConfigureServices` are **not** available for injection.
...
public class MyCommandAvailability : RevitCommandAvailability
{
    ...
    [RevitCommandAvailabilityCheck]
    private bool CheckAvailability(IRevitUiContext context, IMyService myService, MyDependency dep)
    {
